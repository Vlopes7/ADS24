...
def cat_dog(s):
  return True if s.count("dog") == s.count("cat") else False
...
# has22([2, 1, 2]) -> False
def has22(nums):
  return True if '2, 2' in str(nums) else False
...
def soma_na_lista(n, lista):
    return True if any(lista[i] + lista[j] == n for i in range(0,len(lista)) for j in range(i + 1, len(lista))) else False
...
def fila_tijolos(n_peq, n_gra, meta):
  return True if n_peq >= meta % 5 and n_peq + 5 * n_gra >= meta else False
